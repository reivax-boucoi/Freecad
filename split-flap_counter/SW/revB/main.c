...
#define EEPROM_CURRCNT 0
#define EEPROM_MAXCNT 1
...
volatile uint8_t curr_cnt=0;
volatile uint8_t max_cnt=0;
...
int main(void){
	...
	curr_cnt = eeprom_read_byte(EEPROM_CURRCNT);
	max_cnt=eeprom_read_byte(EEPROM_MAXCNT);
	if(curr_cnt>0){
		while(zeroing && remainingStepsM2 > 0);//wait for zeroing to complete
		remainingStepsM2=(curr_cnt/10)*STEPS_PER_FLAP;//goto last stored position
		remainingStepsM1=(curr_cnt%10)*STEPS_PER_FLAP;
    ...
    if(BTN1_STATE){
        ...
        if((btn_debounce & 0x0F)!=0x0){
            ...
            if((btn_debounce & 0x0F) == 0x0){   //Reset counters
                ...
                timer1OVF_cnt = 0;
				curr_cnt = 0;
				eeprom_write_byte(&curr_cnt,EEPROM_CURRCNT);
            }
        }
    }
    ...
    if(BTN2_STATE){
        ...
        if((btn_debounce & 0xF0) != 0x0){
            ...
            if((btn_debounce & 0xF0) == 0x0){
				...
				if(curr_cnt > max_cnt){
					max_cnt=curr_cnt; 
					eeprom_write_byte(&max_cnt,EEPROM_MAXCNT);
				}
			}
        }
    }
