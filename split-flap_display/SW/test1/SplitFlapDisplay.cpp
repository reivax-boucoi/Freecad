...
SplitFlapDisplay::SplitFlapDisplay() { //Constructor
  ...
  for (uint8_t i = 0; i < NUM_MODULES; i++) {
    modules[i] = SplitFlapModule(stepsPerRotation,moduleOffsets[i],magnetPosition);
  }
}
...
void SplitFlapDisplay::testAll() {
  ...
  int targetPositions[NUM_MODULES];
...
void SplitFlapDisplay::testCount() {
  ...
  int count = 0;
  int maxCount = pow(10,NUM_MODULES);
  char targetChar;
  int targetInteger;
  ...
  for (int i = 0; i < maxCount; i++) {
    ...
    for (int j = 0; j < NUM_MODULES; j++) { 
      ...
      targetPositions[NUM_MODULES-j-1] = modules[j].getCharPosition(targetChar);
    }
...
void SplitFlapDisplay::writeString(String inputString,float speed,bool centering) {
  ...
  String displayString = inputString.substring(0, NUM_MODULES);
  
  if (centering) {
    int totalPadding = NUM_MODULES - displayString.length();
    ...
    while (displayString.length() < NUM_MODULES) { // Pad with spaces
...
void SplitFlapDisplay::moveTo(int targetPositions[],float speed, bool releaseMotors) {
  ...
  bool resetLatches[NUM_MODULES]  = {};  // Initialize to false //start with latch on to prevent case where the motion starts with the magnet over the sensor
  bool needsStepping[NUM_MODULES] = {};  // Initialize to false; //modules that still require moving
  unsigned long lastStepTimes[NUM_MODULES] = {};  // Initialize to false; //track when each module was last stepped
  ...
  bool isFinished = checkAllFalse(needsStepping,NUM_MODULES);
  while (!isFinished) {
    ...
    if ((currentTime - lastSensorCheckTime) > checkIntervalUs) { //check hall effect sensor every checkIntervalMs
      ...
      isFinished = checkAllFalse(needsStepping,NUM_MODULES);
